...
model_tar = SimpleNN(input_dim=784, hidden_dim=28).to(device)

criterion = nn.CrossEntropyLoss()

accuracies_tar = []
lr_init = 0.01
optimizer = torch.optim.SGD(model_tar.parameters(), lr=lr_init, weight_decay=0.01) 
...
losses_tar = []
validation_losses_tar = []

num_iterations = 50
# Set a patience value
patience = 10  # Number of iterations to wait for improvement in validation loss
patience_counter = 0  # Counter to track the number of iterations since last improvement
...
for iteration in range(num_iterations):
    ...
    model_tar.train()
    for X_j, y_j in data_loaders['target_train']:
        model_tar.zero_grad()
        X_j, y_j = X_j.to(device), y_j.to(device)
        y_pred_j = model_tar(X_j.view(-1, 784))
        # Compute loss
    ...
    with torch.no_grad(): 
        for X_test, y_test in data_loaders['target_test']:
            ...
            y_pred_test = model_tar(X_test.view(-1, 784))
    ...
    accuracies_tar.append(accuracy_test)
    validation_losses_tar.append(val_loss)

    # Check for early stopping
    if validation_losses_tar[-1] < best_val_loss:
        best_val_loss = validation_losses_tar[-1]
...
np.savetxt(os.path.join(output_directory, 'target_val_loss.txt'), np.array(validation_losses_tar))
np.savetxt(os.path.join(output_directory, 'target_acc.txt'), np.array(accuracies_tar))
